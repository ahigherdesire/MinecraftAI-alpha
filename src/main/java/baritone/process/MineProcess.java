...
public final class MineProcess extends BaritoneProcessHelper implements IMineProcess {
    ...
    /**
     * Symmetric map: given either the stone or deepslate variant of an ore, returns the other.
     * Used so that mineByName("diamond_ore") automatically covers deepslate_diamond_ore too.
     */
    ...
    public void mineByName(int quantity, String... blocks) {
        // Expand stone↔deepslate ore pairs so callers don't need to specify both variants.
        List<BlockOptionalMeta> boms = Stream.of(blocks)
        ...
        for (BlockOptionalMeta bom : boms) {
            ...
            if (counterpart != null && expanded.stream().noneMatch(b -> b.getBlock() == counterpart)) {
                expanded.add(new BlockOptionalMeta(counterpart));
            }
        }
        mine(quantity, new BlockOptionalMetaLookup(expanded.toArray(new BlockOptionalMeta[0])));
    }
