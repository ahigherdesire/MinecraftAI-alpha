...
public enum FasterWorldScanner implements IWorldScanner {
    ...
    public List<BlockPos> scanChunkRadius(IPlayerContext ctx, BlockOptionalMetaLookup filter, int max, int yLevelThreshold, int maxSearchRadius) {
        ...
        int cx = ctx.playerFeet().x >> 4;
        int cz = ctx.playerFeet().z >> 4;
        // Pre-filter to chunks that are actually loaded: avoids iterating thousands of
        // candidate positions that would just return an empty stream inside scanChunkInternal.
        ChunkSource source = ctx.world().getChunkSource();
        List<ChunkPos> loaded = getChunkRange(cx, cz, maxSearchRadius)
                .stream()
                .filter(cp -> source.hasChunk(cp.x(), cp.z()))
                .collect(Collectors.toList());
        return scanChunksInternal(ctx, filter, loaded, max);
    }
    ...
    private void visitSection(BlockOptionalMetaLookup lookup, LevelChunkSection section, List<BlockPos> blocks, long chunkX, int sectionY, long chunkZ) {
        ...
        if (palette instanceof SingleValuePalette) {
            // Entire 16×16×16 section contains only this block type.
            // Emit a 4×4×4 grid of representative positions (one per 4-block sub-cube) so
            // the miner can reach new targets within the section after each block is mined,
            // rather than walking all the way back to the single section-centre every time.
            // 64 positions per section is safe even with unlimited scan budgets.
