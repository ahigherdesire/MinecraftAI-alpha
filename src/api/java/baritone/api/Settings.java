...
public final class Settings {
    ...
    //  Originally planned with auto-eat / auto-flee / auto-torch / autopilot
    //  master toggle; those were dropped because Meteor Client already provides
    //  equivalents. Only auto-sleep is kept here, since it's tightly integrated
    //  with Baritone's bed cache and the #sleep command.
    // ════════════════════════════════════════════════════════════════════════
    ...
    /**
     * After finding this many instances of the target block in the cache, it will stop expanding outward the region
     * search. Raised from 10 → 512 so that when the player is near a region boundary the search continues into
     * adjacent regions and prune() can pick the closest results rather than stopping too early.
     */
    public final Setting<Integer> maxCachedWorldScanCount = new Setting<>(512);
